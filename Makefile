...
typecheck: ## Run type checking with mypy
	uv run mypy google_gmail_tool

test: ## Run tests
	uv run pytest tests/

security-bandit: ## Run bandit security linter
	uv run bandit -r google_gmail_tool -c pyproject.toml
...
run: ## Run google-gmail-tool (usage: make run ARGS="...")
...
uninstall-global: ## Uninstall global installation
	uv tool uninstall google-gmail-tool
